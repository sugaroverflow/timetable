...
* Users can create and recover accounts (sso / magic link).  
* Users can create a new timetable where they have the admin role.  
* Timetables can be given a custom domain (e.g. timetable.2026.newspeak.house)  
* Admins can invite users (paste a list of emails) to their timetables and give and remove various roles (admin, host, elector).  
* A host is someone that can propose topics. An elector is someone that can ❤️ and comment on topics. (A user can be both a host and an elector at the same time).  
...
Admins  
	...
	Timetable is Deactivated (only admins can see it) / Private (only users can see it) / Public (anyone can read electors topic feed & comments)  
	Custom Domain  
...
**👤 User** (name, description, pic, email, isadmin, ishost, iselector, votes, ... , notification settings) (per timetable or global?)

**🏫 Timetable** (name, description, slots, topics, users, public/private, 

**📘 Topic** (name, host(s?), description, votes, creation date, status(live, draft (only visible to host(s)), archived (can no longer be voted on))
...
**❤️ Vote** (elector, topic, issue date, assign date, comment)

**Constitution** (  
	Electors assign votes to topics  
	When a topic assigned to a slot passes, then those votes are refunded

How votes get distributed  
Editable by timetable admins

## **Roles**

**Owner** (can unpublish/freeze their timetable, grant and remove admin privileges, is an admin)  
**Admin**  
**Host**  
**Elector**
